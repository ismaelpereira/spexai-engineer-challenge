...
export class FileProcessorService {
   constructor(
      private readonly fileProcessorRespository: FileProcessorRepository,
   ) {}
   ...
   private async parseCSV(file: Express.Multer.File): Promise<ParsedTripRow[]> {
      ...
      return new Promise((resolve, reject) => {
         ...
         stream
            .pipe(csv())
            .on("data", (data: TripRow) => {
               rows.push({
                  datasource: data.datasource,
                  datetime: new Date(data.datetime),
                  destinationLat: this.parseWktPoint(data.destination_coord)
                     .lat,
                  destinationLon: this.parseWktPoint(data.destination_coord)
                     .lon,
   ...
   private async processTrips(filename: string, rows: ParsedTripRow[]) {
      ...
      for (const row of rows) {
         await this.fileProcessorRespository.createOrUpdateTripsGroup(
            filename,
            {
         ...
         if (processedRows % 100 === 0) {
            await this.fileProcessorRespository.createOrUpdateFileIngestionStatus(
               filename,
               {
                  status: "processing",
                  startedAt: new Date(),
